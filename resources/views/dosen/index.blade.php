...
@section('konten')
    <div class="flex justify-between mx-8 my-4">
        <div class="flex">
            ...
            <form class="" action="{{ url('dosen') }}" method="GET">
            ...
            <tbody>
                ...
                @foreach ($data as $item)
                    <tr
                        ...
                        <td class="px-3 py-4">
                            @if ($item->foto)
                                <img class="w-10 h-10 rounded-full" src="{{ url('foto') . '/' . $item->foto }}"
                                    alt="Rounded avatar">
                            @endif
                        </td>
                        <td class="px-6 py-4 font-medium text-green-900 whitespace-nowrap">
                            {{ $item->nama }}</td>
                        <td class="px-4 py-4">{{ $item->nidn }}</td>
                        <td class="px-6 py-4">{{ $item->alamat }}</td>
                        <td class=" py-4">
                            <a href='{{ url('dosen/' . $item->nidn . '/edit') }}'
                            ...
                            <form onsubmit="return confirm('Yakin Akan Menghapus Data?')"
                                action="{{ url('dosen/' . $item->nidn) }}" method="POST"
