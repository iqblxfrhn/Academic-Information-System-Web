...
@if ($paginator->hasPages())
    <nav role="navigation" aria-label="Pagination Navigation"
        class="flex items-center justify-between p-4 border-t select-none border-utama sm:px-6">
        <div class="flex justify-between flex-1 sm:hidden">
            ...
            @else
                ...
                <a href="{{ $paginator->previousPageUrl() }}"
                    class="relative inline-flex items-center px-4 py-2 text-sm font-medium leading-5 transition bg-white border border-gray-300 rounded hover:text-hijau focus:outline-none focus:border-primary-300 focus:ring focus:ring-hijau focus:ring-opacity-30">
                    {!! __('pagination.previous') !!}
                </a>
            @endif
            {{-- next enable --}}
                ...
                <span class="relative z-0 inline-flex mt-2 shadow-sm lg:mt-0">
                    ...
                    @else
                        ...
                        <a href="{{ $paginator->previousPageUrl() }}" rel="prev"
                            class="relative inline-flex items-center px-2 py-2 text-sm font-medium leading-5 transition bg-white border border-gray-300 rounded-l hover:text-hijau focus:z-10 focus:outline-none focus:border-primary-300 focus:ring focus:ring-hijau focus:ring-opacity-30"
                        ...
                        @if (is_array($element))
                            @foreach ($element as $page => $url)
                                ...
                                @else
                                    ...
                                    <a href="{{ $url }}"
                                        class="relative inline-flex items-center px-4 py-2 -ml-px text-sm font-medium leading-5 transition bg-white border border-gray-300 hover:text-hijau focus:z-10 focus:outline-none focus:border-primary-300 focus:ring focus:ring-hijau focus:ring-opacity-30"
                    ...
                    @if ($paginator->hasMorePages())
                        <a href="{{ $paginator->nextPageUrl() }}" rel="next"
                            class="relative inline-flex items-center px-2 py-2 -ml-px text-sm font-medium leading-5 transition bg-white border border-gray-300 rounded-r hover:text-hijau focus:z-10 focus:outline-none focus:border-primary-300 focus:ring focus:ring-hijau focus:ring-opacity-30"
